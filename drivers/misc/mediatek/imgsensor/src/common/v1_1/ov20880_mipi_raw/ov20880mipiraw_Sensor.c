...
{
	printk("ov20880 image_mirror = %d\n", image_mirror);
	pr_debug("ov20880 0x3820 = %d, 0x3674 = %d\n", read_cmos_sensor(0x3820), read_cmos_sensor(0x3674));
...
{
	printk("ov20880 -- sensor_init enter 0511\n");
	/* OV20880 4c setting */
...
{
	printk("ov20880 -- preview_init enter\n");
#if 1
...
{
	printk("ov20880 -- capture_init enter\n");
#if 1
write_cmos_sensor(0x340c, 0x0f);
write_cmos_sensor(0x340d, 0xe0);
write_cmos_sensor(0x3501, 0xf9);
...
{
	pr_debug("ov20880 -- normal_video_setting! currefps:%d\n", currefps);
...
static void hs_video_setting(void)
{
	...
	printk("ov20880 -- hs_video_init enter\n");
#if 1
write_cmos_sensor(0x340c, 0x08);
write_cmos_sensor(0x340d, 0x38);
write_cmos_sensor(0x3501, 0x7f);
...
{
	pr_debug("ov20880 -- slim_video_setting\n");
	//preview_setting();
write_cmos_sensor(0x340c, 0x08);
write_cmos_sensor(0x340d, 0x38);
write_cmos_sensor(0x3501, 0x7f);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	printk("ov20880 get_imgsensor_id ++++++++++++++++ \n");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
					printk("OV20880 i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x,imgsensor_info.sensor_id=0x%x\n",imgsensor.i2c_write_id, *sensor_id, vendor_id,imgsensor_info.sensor_id);

				return ERROR_NONE;
				} else {
					printk("OV20880 check id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x,imgsensor_info.sensor_id=0x%x\n",imgsensor.i2c_write_id, *sensor_id,vendor_id,imgsensor_info.sensor_id);
					*sensor_id = 0xFFFFFFFF;
				}
			pr_debug("ov20880 Read sensor id fail:0x%x, id: 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	printk("open PLATFORM:MIPI 4LANE ov20880+++++ ++++\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("ov20880 open i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
		    printk("ov20880 open Read sensor id fail, id: 0x%x,sensor_id =0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	{
		pr_debug("ov20880 SENSOR_FEATURE_GET_MIPI_PIXEL_RATE\n");
		switch (*feature_data) {
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		...
		pr_debug("ov20880 SENSOR_FEATURE_GET_MIPI_PIXEL_RATE wangweifeng:rate:%d\n",rate);
...
UINT32 OV20880_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	printk("OV20880_MIPI_RAW_SensorInit 0522_20M_30fps !!!!!\n");
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
