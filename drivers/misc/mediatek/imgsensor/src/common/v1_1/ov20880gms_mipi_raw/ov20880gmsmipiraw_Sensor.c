...
#define PFX "ov20880gmsmipiraw_Sensor"
...
*/
#include "ov20880gmsmipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,//SENSOR_OUTPUT_FORMAT_RAW_4CELL_B,//sensor output first pixel color

	.mclk = 24,	/* mclk value, suggest 24 or 26 for 24Mhz or 26Mhz */
	.mipi_lane_num = SENSOR_MIPI_4_LANE,	/* mipi lane num */

	.i2c_addr_table = { 0x20, 0x42, 0x6C, 0xff},
...
static struct imgsensor_struct imgsensor = {
	//0518_3 IMAGE_NORMAL B   IMAGE_H_MIRROR Gb IMAGE_V_MIRROR Gr IMAGE_HV_MIRROR R      
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	{5184, 3888, 0, 0, 5184, 3888, 5184,
	 3888, 0, 0, 5184, 3888, 0, 0, 5184, 3888},	/* preview */
	...
	{5184, 3888, 0, 0, 5184, 3888, 2592,
	 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944},	/* hight speed video */
	{5184, 3888, 0, 0, 5184, 3888, 2592,
...
{
	printk("ov20880gms image_mirror = %d\n", image_mirror);
	pr_debug("ov20880gms 0x3820 = %d, 0x3674 = %d\n", read_cmos_sensor(0x3820), read_cmos_sensor(0x3674));
    ...
    switch (image_mirror) {
		case IMAGE_NORMAL:
				write_cmos_sensor(0x3820, (0xfb & (read_cmos_sensor(0x3820))));	//Flip_normal R3820[2]=0
				write_cmos_sensor(0x4500, (0x7f & (read_cmos_sensor(0x4500))));
				//write_cmos_sensor(0x3813, (((read_cmos_sensor(0x3813) + 1) / 2) * 2));
				write_cmos_sensor(0x3674, (0x04 | (read_cmos_sensor(0x3674))));	//Mirror_normal R3674[2]=1
				//write_cmos_sensor(0x3811, (((read_cmos_sensor(0x3811) + 1) / 2) * 2 - 1));
			break;
		case IMAGE_H_MIRROR:
				write_cmos_sensor(0x3820, (0xfb & (read_cmos_sensor(0x3820))));	//Flip_normal R3820[2]=0
				write_cmos_sensor(0x4500, (0x7f & (read_cmos_sensor(0x4500))));
				//write_cmos_sensor(0x3813, (((read_cmos_sensor(0x3813) + 1) / 2) * 2));
				write_cmos_sensor(0x3674, (0xfb & (read_cmos_sensor(0x3674))));	//Mirror_Mirror R3674[2]=0
				//write_cmos_sensor(0x3811, (((read_cmos_sensor(0x3811) + 1) / 2) * 2));
			break;
		case IMAGE_V_MIRROR:
				write_cmos_sensor(0x3820, (0x04 | (read_cmos_sensor(0x3820))));	//Flip_Flip R3820[2]=1
				write_cmos_sensor(0x4500, (0x80 | (read_cmos_sensor(0x4500))));
				//write_cmos_sensor(0x3813, (((read_cmos_sensor(0x3813) + 1) / 2) * 2 - 1));
				write_cmos_sensor(0x3674, (0x04 | (read_cmos_sensor(0x3674))));	//Mirror_normal R3674[2]=1
				//write_cmos_sensor(0x3811, (((read_cmos_sensor(0x3811) + 1) / 2) * 2 - 1));
			break;
...
{
	printk("ov20880gms -- sensor_init enter 0511\n");
	/* OV20880GMS 4c setting */
	write_cmos_sensor(0x0103, 0x01);	/* SW Reset, need delay */
	mdelay(10);
write_cmos_sensor(0x0300, 0x06); 
write_cmos_sensor(0x0301, 0xe1); 
write_cmos_sensor(0x0302, 0x14); 
write_cmos_sensor(0x0303, 0x01); 
write_cmos_sensor(0x0304, 0xc0); 
write_cmos_sensor(0x0305, 0x03); 
write_cmos_sensor(0x0314, 0x06); 
write_cmos_sensor(0x0315, 0x01); 
write_cmos_sensor(0x0316, 0xe0); 
...
write_cmos_sensor(0x3415, 0x10); 
write_cmos_sensor(0x3500, 0x00); 
write_cmos_sensor(0x3501, 0xf9); 
...
write_cmos_sensor(0x4d01, 0xeb); 
write_cmos_sensor(0x4d02, 0xb9); 
write_cmos_sensor(0x4d03, 0x01); 
write_cmos_sensor(0x4d04, 0x10); 
write_cmos_sensor(0x4d05, 0x58); 
write_cmos_sensor(0x5000, 0x8b); 
write_cmos_sensor(0x5001, 0x43); //0x53 05-22
...
write_cmos_sensor(0x5022, 0x14); 
write_cmos_sensor(0x5023, 0x50); 
write_cmos_sensor(0x5024, 0x00); 
write_cmos_sensor(0x5025, 0x08); 
write_cmos_sensor(0x5026, 0x0f); 
write_cmos_sensor(0x5027, 0x48); 
...
write_cmos_sensor(0x5301, 0x08); 
write_cmos_sensor(0x5302, 0x00); 
write_cmos_sensor(0x5303, 0x0a); 
write_cmos_sensor(0x5304, 0x00); 
write_cmos_sensor(0x5305, 0x10); 
write_cmos_sensor(0x5306, 0x00); 
write_cmos_sensor(0x5307, 0x18); 
write_cmos_sensor(0x5308, 0x00); 
write_cmos_sensor(0x5309, 0x1e); 
write_cmos_sensor(0x530a, 0x00); 
write_cmos_sensor(0x530b, 0x26); 
write_cmos_sensor(0x534a, 0x07); 
write_cmos_sensor(0x534f, 0x10); 
write_cmos_sensor(0x5404, 0x00); 
write_cmos_sensor(0x5405, 0x00); 
write_cmos_sensor(0x5406, 0x02); 
write_cmos_sensor(0x5407, 0xc0); 
write_cmos_sensor(0x5408, 0x03); 
write_cmos_sensor(0x5409, 0xa8); 
write_cmos_sensor(0x540a, 0x00); 
write_cmos_sensor(0x540b, 0x00); 
...
write_cmos_sensor(0x59b9, 0x64); 
write_cmos_sensor(0x59ba, 0x00); 
write_cmos_sensor(0x59bb, 0x86); 
...
write_cmos_sensor(0x3032, 0x0a); 
write_cmos_sensor(0x3016, 0xf1); 
write_cmos_sensor(0x3018, 0xf4); 
write_cmos_sensor(0x3012, 0x45); 
//write_cmos_sensor(0x0100, 0x01); 
write_cmos_sensor(0x3012, 0x41); 
write_cmos_sensor(0x3032, 0x0e); 
write_cmos_sensor(0x3018, 0xf0); 
write_cmos_sensor(0x3016, 0xf0); 
//write_cmos_sensor(0x0100, 0x01);
}
...
{
	printk("ov20880gms -- preview_init enter\n");
#if 1
...
{
	printk("ov20880gms -- capture_init enter\n");
#if 1
write_cmos_sensor(0x340c, 0x0f);
write_cmos_sensor(0x340d, 0xe0);
write_cmos_sensor(0x3501, 0xf9);
...
{
	pr_debug("ov20880gms -- normal_video_setting! currefps:%d\n", currefps);
	//pr_debug("E! video just has 30fps preview size setting ,NOT HAS 24FPS SETTING!\n");
	//preview_setting();
write_cmos_sensor(0x340c, 0x08);
write_cmos_sensor(0x340d, 0x38);
write_cmos_sensor(0x3501, 0x7f);
write_cmos_sensor(0x3511, 0x1f);
write_cmos_sensor(0x3602, 0x82);
//write_cmos_sensor(0x3661, 0x0f);
write_cmos_sensor(0x366c, 0x10);
...
write_cmos_sensor(0x5800, 0xf1);
write_cmos_sensor(0x5801, 0x04);
...
static void hs_video_setting(void)
{
	...
	printk("ov20880gms -- hs_video_init enter\n");
#if 1
write_cmos_sensor(0x340c, 0x08);
write_cmos_sensor(0x340d, 0x38);
write_cmos_sensor(0x3501, 0x7f);
...
{
	pr_debug("ov20880gms -- slim_video_setting\n");
	//preview_setting();
write_cmos_sensor(0x340c, 0x08);
write_cmos_sensor(0x340d, 0x38);
write_cmos_sensor(0x3501, 0x7f);
...
{
	return (((read_cmos_sensor(0x300a) << 16) | (read_cmos_sensor(0x300b) << 8) | read_cmos_sensor(0x300c)) & 0xFFFFFF);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	printk("ov20880gms get_imgsensor_id ++++++++++++++++ \n");
		...
		do {
			*sensor_id = return_sensor_id()+1;
			if (*sensor_id == imgsensor_info.sensor_id) {
					printk("OV20880GMS i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x,imgsensor_info.sensor_id=0x%x\n",imgsensor.i2c_write_id, *sensor_id, vendor_id,imgsensor_info.sensor_id);

				return ERROR_NONE;
				} else {
					printk("OV20880GMS check id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x,imgsensor_info.sensor_id=0x%x\n",imgsensor.i2c_write_id, *sensor_id,vendor_id,imgsensor_info.sensor_id);
					*sensor_id = 0xFFFFFFFF;
				}
			pr_debug("ov20880gms Read sensor id fail:0x%x, id: 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	printk("open PLATFORM:MIPI 4LANE ov20880gms+++++ ++++\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id()+1;
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("ov20880gms open i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
		    printk("ov20880gms open Read sensor id fail, id: 0x%x,sensor_id =0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	printk("ov20880gms feature_id = %d\n", feature_id);
	switch (feature_id) {
		case SENSOR_FEATURE_GET_AWB_REQ_BY_SCENARIO:	
			...
			break;
	case SENSOR_FEATURE_GET_GAIN_RANGE_BY_SCENARIO:   // 1
		*(feature_data + 1) = imgsensor_info.min_gain;
		*(feature_data + 2) = imgsensor_info.max_gain;
		break;
	case SENSOR_FEATURE_GET_BASE_GAIN_ISO_AND_STEP:   // 2
		*(feature_data + 0) = imgsensor_info.min_gain_iso;
		*(feature_data + 1) = imgsensor_info.gain_step;
		*(feature_data + 2) = imgsensor_info.gain_type;
		break;
	case SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO:   // 3
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:   //4
		...
		*feature_para_len = 4;
		...
		break;
	case SENSOR_FEATURE_GET_FRAME_CTRL_INFO_BY_SCENARIO:   // 5
		(*(feature_data + 1)) = 1;
		(*(feature_data + 2)) = imgsensor_info.margin;
		break;
		
     case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ_BY_SCENARIO:    //6
        switch (*feature_data) {
            case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:   // 7
		pr_debug("SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO \n");
		switch (*feature_data) {
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1))
	...
	{
		pr_debug("ov20880gms SENSOR_FEATURE_GET_MIPI_PIXEL_RATE\n");
		switch (*feature_data) {
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		...
		pr_debug("ov20880gms SENSOR_FEATURE_GET_MIPI_PIXEL_RATE wangweifeng:rate:%d\n",rate);
...
UINT32 OV20880GMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	printk("OV20880GMS_MIPI_RAW_SensorInit 0522_20M_30fps !!!!!\n");
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
