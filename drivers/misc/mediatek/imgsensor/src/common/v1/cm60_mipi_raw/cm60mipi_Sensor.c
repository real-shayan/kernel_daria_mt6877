...
#include "cm60mipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "cm60_camera_sensor"
#define LOG_1 LOG_INF("CM60MIPI, 2LANE\n")
/****************************   Modify end    *******************************************/
#define CM60_DEBUG 1
#if CM60_DEBUG
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = CM60_SENSOR_ID,
	.checksum_value = 0xf7375923,

	.pre = {
		.pclk = 12000000,   //drv add by lipengpeng 20230829 start 
		.linelength = 1488,
		.framelength = 827,
		.startx = 0,
		.starty = 0,
		...
		.max_framerate = 600,  //drv add by lipengpeng 20230829 start 
	},
	.cap = {
		.pclk = 12000000,
		.linelength = 1488,
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO,//SENSOR_OUTPUT_FORMAT_RAW_B,
#else
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	printk("[read_cmos_sensor]i2c write id: 0x%x, addr id: 0x%x\n",
						imgsensor.i2c_write_id, pu_send_cmd[0]);
...
{
	return read_cmos_sensor(CM60_CHIP);
}
...
static void sensor_init(void)
{
	LOG_INF("E\n");
	/* SYSTEM */
//drv add by lipengpeng 20230829 start 
	// set reg 0xc5 to 0x02, switch to 1 lane.
//	write_cmos_sensor(0xc5, 0x02);
	// switching cm60 lane mode need some delay.
//	mdelay(30);
//drv add by lipengpeng 20230829 end 
}

#define STREAM_ENABLE       1

#if STREAM_ENABLE
void CM60StreamOn(void)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				printk("[cm60_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			printk("[cm60_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[cm60_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_debug("[cm60_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
UINT32 CM60_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
