...
extern int gt9xx_tpd_debug_time;
...
#define TPD_DEBUG_PRINT_UP                           \
	do {                                                              \
		if (pending == 0 && gt9xx_tpd_debug_time) {        \
			tpd_down_status = 0;                \
...
#define GT9XX_TPD_DEBUG_SET_TIME   do { gt9xx_tpd_debug_set_time(); } while (0)

extern int gt9xx_tpd_em_log;
extern int gt9xx_tpd_em_log_to_fs;
extern int tpd_type_cap;

void gt9xx_tpd_em_log_output(int raw_x,
	int raw_y, int cal_x, int cal_y, int p, int down);
void gt9xx_tpd_em_log_store(int raw_x,
	int raw_y, int cal_x, int cal_y, int p, int down);
...
#define TPD_EM_PRINT(raw_x, raw_y, cal_x, cal_y, p, down)\
	do {                             \
		if (gt9xx_tpd_em_log) {\
			if (!gt9xx_tpd_em_log_to_fs) {        \
				gt9xx_tpd_em_log_output(raw_x, \
				raw_y, cal_x, cal_y, p, down);             \
			} else {                        \
				gt9xx_tpd_em_log_store(raw_x, \
					raw_y, cal_x, cal_y, p, down);   \
				gt9xx_tpd_em_log_output(raw_x, \
				raw_y, cal_x, cal_y, p, down); \
			} \
			...
			if (gt9xx_tpd_em_log_to_fs) {                        \
				gt9xx_tpd_em_log_release(); \
			}                                            \
		}                                               \
		GT9XX_MET_touch(raw_x, raw_y, cal_x, cal_y, p, down);               \
	} while (0)
