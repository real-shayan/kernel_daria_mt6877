...
int tpd_down_status;
module_param(gt9xx_tpd_debug_time, int, 0644);

void gt9xx_tpd_debug_set_time(void)
{
	struct timeval t;

	if (!gt9xx_tpd_debug_time && !gt9xx_tpd_em_log)
		return;
...
int gt9xx_tpd_em_log;
module_param(gt9xx_tpd_em_log, int, 0664);

void gt9xx_tpd_enable_em_log(int enable)
{
	if (enable)
		gt9xx_tpd_em_log = 1;
	else
...
EXPORT_SYMBOL(gt9xx_tpd_enable_em_log);


int gt9xx_tpd_em_log_to_fs;
module_param(gt9xx_tpd_em_log_to_fs, int, 0664);
...
static int tpd_debug_log_open(struct inode *inode, struct file *file)
{
	...
	pr_debug("[gt9xx_tpd_em_log]: open log file\n");
	return 0;
}
...
static int tpd_debug_log_release(struct inode *inode, struct file *file)
{
	...
	pr_debug("[gt9xx_tpd_em_log]: close log file\n");
	spin_lock(&tpd_buf.buffer_lock);
...
static unsigned char *tpd_log_find_buffer(void)
{
	...
	if (tpd_buf.buffer == NULL) {
		pr_info("[gt9xx_tpd_em_log] :tpd_buf.buffer is NULL\n");
		return NULL;
	}
	spin_lock(&tpd_buf.buffer_lock);
	...
	spin_unlock(&tpd_buf.buffer_lock);
	if (tpd_buf.head == tpd_buf.tail) {
		snprintf(buffer, unit, "[gt9xx_tpd_em_log] overlay !!!!!\n");
		return NULL;
	}
...
static struct miscdevice tpd_debug_log_dev = {
	...
	.name = "gt9xx_tpd_em_log",
	.fops = &tpd_debug_log_fops,
};
#ifndef CREATE_TRACE_POINTS
#define CREATE_TRACE_POINTS
#endif
noinline void GT9XX_MET_touch(int raw_x, int raw_y,
...
void gt9xx_tpd_em_log_output(int raw_x, int raw_y,
			int cal_x, int cal_y, int p, int down)
{
	if (down == TPD_TYPE_INT_DOWN) {
		pr_debug("[gt9xx_tpd_em_log] int trigger down\n");
	} else if (down == TPD_TYPE_INT_UP) {
		pr_debug("[gt9xx_tpd_em_log] int trigger up\n");
	} else if (down == TPD_TYPE_TIMER) {
		pr_debug("[gt9xx_tpd_em_log] timer trigger\n");
	} else if (down == TPD_TYPE_RAW_DATA) {
		if (gt9xx_tpd_em_log == TPD_TYPE_RAW_DATA) {
			pr_debug("[gt9xx_tpd_em_log] rx=%d,ry=%d,rz1=%d"
				SPLIT "rz2=%d,p=%d,r\n",
			       raw_x, raw_y, cal_x, cal_y, p);
		}
	} else if (down == TPD_TYPE_REJECT1) {
		pr_debug("[gt9xx_tpd_em_log] the first or last point is rejected\n");
	} else if (down == TPD_TYPE_REJECT2) {
		pr_debug
		    ("[gt9xx_tpd_em_log] pressure(%d) > NICE_PRESSURE(%d)"
		    SPLIT "debounce debt0:%d ms, debt1:%d ms, spl_num:%d\n",
		     raw_x, raw_y, cal_x, cal_y, p);
	} else if (down == TPD_TYPE_FIST_LATENCY) {
		pr_debug("[gt9xx_tpd_em_log] The first touch latency is %d ms\n",
			raw_x / 1000);
	} else if (down && tpd_down_status == 0) {
		pr_debug("[gt9xx_tpd_em_log] rx=%d,ry=%d,cx=%d"
...
void gt9xx_tpd_em_log_store(int raw_x, int raw_y,
			int cal_x, int cal_y, int p, int down)
{
	...
	if (down == TPD_TYPE_INT_DOWN) {
		snprintf(buffer, tpd_log_line_buffer,
			"[%5lu.%06lu][gt9xx_tpd_em_log] int trigger down\n",
			 (t.tv_sec & 0xFFF), t.tv_usec);
	} else if (down == TPD_TYPE_INT_UP) {
		snprintf(buffer, tpd_log_line_buffer,
			"[%5lu.%06lu][gt9xx_tpd_em_log] int trigger up\n",
			 (t.tv_sec & 0xFFF), t.tv_usec);
	} else if (down == TPD_TYPE_TIMER) {
		snprintf(buffer, tpd_log_line_buffer,
			"[%5lu.%06lu][gt9xx_tpd_em_log] timer trigger\n",
			 (t.tv_sec & 0xFFF), t.tv_usec);
	} else if (down == TPD_TYPE_RAW_DATA) {
		if (gt9xx_tpd_em_log == TPD_TYPE_RAW_DATA) {
			snprintf(buffer, tpd_log_line_buffer,
				 "[%5lu.%06lu][gt9xx_tpd_em_log]"
...
void gt9xx_tpd_em_log_release(void)
{
	struct tpd_em_log_struct *p, *p_tmp;

	if (!tpd_em_log_first) {
		remove_proc_entry("gt9xx_tpd_em_log", NULL);
...
int tpd_log_init(void)
{
	if (misc_register(&tpd_debug_log_dev) < 0) {
		pr_info("[gt9xx_tpd_em_log] :register device failed\n");
		return -1;
	}
	pr_info("[gt9xx_tpd_em_log] :register device successfully\n");
