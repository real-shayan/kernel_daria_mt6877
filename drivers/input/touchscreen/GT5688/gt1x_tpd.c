...
static u8	hw_chip_type =0;//prize add by kuangliangjun 20230726 end
...
/*prize add by kuangliangjun 20230726--------start*/	
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	gt1x_i2c_read_dbl_check(GTP_REG_SENSOR_ID, &sensor_id, 1);
	gt1x_i2c_read_dbl_check(GTP_REG_CONFIG_DATA, &ctp_fw_version, 1);
	...
	if(hw_chip_type==1)
		...
		sprintf(current_tp_info.chip,"GT1151 fw_version:0x%02x", ctp_fw_version);
    sprintf(current_tp_info.id,"0x%04x",sensor_id);
    strcpy(current_tp_info.vendor,"goodix");
    sprintf(current_tp_info.more,"%d*%d",720,1280);
